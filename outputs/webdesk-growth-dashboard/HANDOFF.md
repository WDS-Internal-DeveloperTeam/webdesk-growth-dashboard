...
> `(shell)` layout's session gate. **Phase 1F's own schema migrations (`00034`/`00035`) have not
> been run against the real production database yet** — same "user runs it themselves" pattern as
> every prior phase's production migration. See `docs/project-state/phase-1f-validation-report.md`
> and `docs/project-state/phase-1f-approval-checklist.md`'s "Sign-off" section for the full record.
