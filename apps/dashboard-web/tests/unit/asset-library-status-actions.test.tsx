...
import { AssetLibraryStatusActions } from "../../components/asset-library-status-actions.js";

const ASSET_ID = "11111111-1111-1111-1111-111111111111";

describe("AssetLibraryStatusActions", () => {
  ...
  it("draft: renders Submit for Review and Archive", () => {
    render(<AssetLibraryStatusActions assetId={ASSET_ID} approvalStatus="draft" />);
    expect(screen.getByRole("button", { name: "Submit for Review" })).toBeInTheDocument();
    expect(screen.getByRole("button", { name: "Archive" })).toBeInTheDocument();
  });

  it("under_review: renders Approve, Request Revision, Reject, and Archive", () => {
    render(<AssetLibraryStatusActions assetId={ASSET_ID} approvalStatus="under_review" />);
  ...
  it("rejected: includes Revert to Draft — the fully-recoverable resubmit path, no confirmation needed", () => {
    render(<AssetLibraryStatusActions assetId={ASSET_ID} approvalStatus="rejected" />);
  ...
  it("archived: renders nothing — archived is a terminal state", () => {
    const { container } = render(
      <AssetLibraryStatusActions assetId={ASSET_ID} approvalStatus="archived" />,
    );
  ...
  it("superseded: renders nothing — superseded is a terminal state", () => {
    const { container } = render(
      <AssetLibraryStatusActions assetId={ASSET_ID} approvalStatus="superseded" />,
    );
    expect(container).toBeEmptyDOMElement();
  });
  ...
  it("Submit for Review: posts {approvalStatus: submitted} with no confirmation prompt, then refreshes", async () => {
    ...
    expect(fetchMock).toHaveBeenCalledWith(
      `https://api.example.com/asset-library/assets/${ASSET_ID}/status`,
    ...
    global.fetch = vi.fn().mockResolvedValue({
      ...
      json: async () => ({
        ...
        error: {
          ...
          message: `Asset ${ASSET_ID} approval status changed concurrently (expected draft, now submitted) — reload and retry`,
        },
  ...
  it("re-syncs approvalStatus from a fresh prop (e.g. after a sibling AssetLibraryPublishActions transition triggers router.refresh()) without a remount", () => {
    const { rerender } = render(
      <AssetLibraryStatusActions assetId={ASSET_ID} approvalStatus="draft" />,
    );
    expect(screen.getByRole("button", { name: "Submit for Review" })).toBeInTheDocument();

    rerender(<AssetLibraryStatusActions assetId={ASSET_ID} approvalStatus="approved" />);
