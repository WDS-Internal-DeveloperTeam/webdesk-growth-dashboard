...
import { WireframeStatusActions } from "../../components/wireframe-status-actions.js";

const RECORD_ID = "11111111-1111-1111-1111-111111111111";

describe("WireframeStatusActions", () => {
  ...
  it("draft: renders Submit for Review and Archive", () => {
    render(<WireframeStatusActions recordId={RECORD_ID} approvalStatus="draft" />);
    expect(screen.getByRole("button", { name: "Submit for Review" })).toBeInTheDocument();
    expect(screen.getByRole("button", { name: "Archive" })).toBeInTheDocument();
  });

  it("under_review: renders Approve, Request Revision, Reject, and Archive", () => {
    render(<WireframeStatusActions recordId={RECORD_ID} approvalStatus="under_review" />);
  ...
  it("approved: renders ONLY Archive — this module's own deliberate divergence from every sibling module, which also offers Mark as Superseded here", () => {
    render(<WireframeStatusActions recordId={RECORD_ID} approvalStatus="approved" />);
    expect(screen.getByRole("button", { name: "Archive" })).toBeInTheDocument();
  ...
  it("rejected: includes Revert to Draft — the fully-recoverable resubmit path, no confirmation needed", () => {
    render(<WireframeStatusActions recordId={RECORD_ID} approvalStatus="rejected" />);
  ...
  it("archived: renders nothing — archived is a terminal state", () => {
    const { container } = render(
      <WireframeStatusActions recordId={RECORD_ID} approvalStatus="archived" />,
    );
  ...
  it("superseded: renders nothing — superseded is a terminal state", () => {
    const { container } = render(
      <WireframeStatusActions recordId={RECORD_ID} approvalStatus="superseded" />,
    );
    expect(container).toBeEmptyDOMElement();
  });
  ...
  it("Submit for Review: posts {approvalStatus: submitted} with no confirmation prompt, then refreshes", async () => {
    ...
    expect(fetchMock).toHaveBeenCalledWith(
      `https://api.example.com/wireframe-library/records/${RECORD_ID}/status`,
    ...
    global.fetch = vi.fn().mockResolvedValue({
      ...
      json: async () => ({
        ...
        error: {
          ...
          message: `Wireframe record ${RECORD_ID} approval status changed concurrently (expected draft, now submitted) — reload and retry`,
        },
  ...
  it("renders nothing for a status outside the known union, instead of throwing", () => {
    const unknownStatus = "not_a_real_status" as unknown as Parameters<
      typeof WireframeStatusActions
    >[0]["approvalStatus"];
    const { container } = render(
      <WireframeStatusActions recordId={RECORD_ID} approvalStatus={unknownStatus} />,
    );
    expect(container).toBeEmptyDOMElement();
  });
});
