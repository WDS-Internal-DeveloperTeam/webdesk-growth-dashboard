...
import { TechnicalCheckRunStatusActions } from "../../components/technical-check-run-status-actions.js";

const PROJECT_ID = "11111111-1111-1111-1111-111111111111";
const RUN_ID = "22222222-2222-2222-2222-222222222222";
const STATUS_URL = `https://api.example.com/technical-center/projects/${PROJECT_ID}/runs/${RUN_ID}/status`;

describe("TechnicalCheckRunStatusActions", () => {
  ...
  it("requested: renders Queue and Cancel run", () => {
    render(
      <TechnicalCheckRunStatusActions projectId={PROJECT_ID} runId={RUN_ID} status="requested" />,
    );
  ...
  it("running: renders all 5 outbound transitions", () => {
    render(
      <TechnicalCheckRunStatusActions projectId={PROJECT_ID} runId={RUN_ID} status="running" />,
    );
  ...
  it.each(["completed", "partially_completed", "failed", "timed_out", "cancelled"] as const)(
    ...
    (status) => {
      const { container } = render(
        <TechnicalCheckRunStatusActions projectId={PROJECT_ID} runId={RUN_ID} status={status} />,
      );
  ...
  it("queued -> running submits immediately with no extra data, then refreshes", async () => {
    ...
    render(
      <TechnicalCheckRunStatusActions projectId={PROJECT_ID} runId={RUN_ID} status="queued" />,
    );
  ...
  it("failed: opens an inline form with an optional error summary, and includes it only if non-empty", async () => {
    ...
    fireEvent.change(screen.getByLabelText("Error summary (optional)"), {
      target: { value: "ESLint exited with a non-zero status" },
    });
    ...
    await waitFor(() =>
      expect(fetchMock).toHaveBeenCalledWith(
        ...
        expect.objectContaining({
          body: JSON.stringify({
            status: "failed",
            errorSummary: "ESLint exited with a non-zero status",
          }),
        }),
      ),
    );
  ...
  it("completed: opens a findings editor, drops rows with a blank title, and sends the rest", async () => {
    ...
    fireEvent.change(titleInputs[0]!, { target: { value: "Unused import in main.ts" } });
    ...
    expect(body.findings?.[0]?.title).toBe("Unused import in main.ts");
  });
    ...
    global.fetch = vi.fn().mockResolvedValue({
      ...
      json: async () => ({
        ...
        error: {
          ...
          message: `Technical check run ${RUN_ID} status changed concurrently (expected requested, now queued) — reload and retry`,
        },
  ...
  it("renders nothing for a status outside the known union, instead of throwing", () => {
    const unknownStatus = "not_a_real_status" as unknown as Parameters<
      typeof TechnicalCheckRunStatusActions
    >[0]["status"];
    const { container } = render(
