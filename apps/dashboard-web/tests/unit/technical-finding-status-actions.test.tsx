...
import { TechnicalFindingStatusActions } from "../../components/technical-finding-status-actions.js";

const PROJECT_ID = "11111111-1111-1111-1111-111111111111";
const FINDING_ID = "22222222-2222-2222-2222-222222222222";
const STATUS_URL = `https://api.example.com/technical-center/projects/${PROJECT_ID}/findings/${FINDING_ID}/status`;

describe("TechnicalFindingStatusActions", () => {
  ...
  it("open: renders Acknowledge, Resolve, and Dismiss", () => {
    render(
      <TechnicalFindingStatusActions projectId={PROJECT_ID} findingId={FINDING_ID} status="open" />,
    );
  ...
  it("shows the backend's error message on failure, without refreshing", async () => {
    global.fetch = vi.fn().mockResolvedValue({
      ...
      json: async () => ({
        ...
        error: {
          ...
          message: "Invalid technical finding status transition",
        },
    ...
    expect(await screen.findByRole("alert")).toHaveTextContent(/Invalid technical finding status/);
    expect(refreshMock).not.toHaveBeenCalled();
  });
  ...
  it("renders nothing for a status outside the known union, instead of throwing", () => {
    const unknownStatus = "not_a_real_status" as unknown as Parameters<
      typeof TechnicalFindingStatusActions
    >[0]["status"];
    const { container } = render(
      <TechnicalFindingStatusActions
        projectId={PROJECT_ID}
