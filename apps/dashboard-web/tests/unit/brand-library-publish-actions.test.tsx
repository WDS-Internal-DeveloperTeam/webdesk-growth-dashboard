...
import { BrandLibraryPublishActions } from "../../components/brand-library-publish-actions.js";

const RECORD_ID = "11111111-1111-1111-1111-111111111111";

describe("BrandLibraryPublishActions", () => {
  ...
  it("Publish: posts to the publish route with no confirmation prompt, then refreshes", async () => {
    ...
    expect(fetchMock).toHaveBeenCalledWith(
      `https://api.example.com/brand-library/records/${RECORD_ID}/publish`,
  ...
  it("Unpublish: posts to the unpublish route with no confirmation prompt (still approved), then refreshes", async () => {
    ...
    expect(confirmSpy).not.toHaveBeenCalled();
    expect(fetchMock).toHaveBeenCalledWith(
      `https://api.example.com/brand-library/records/${RECORD_ID}/unpublish`,
      expect.objectContaining({ method: "POST", credentials: "include" }),
    );
    await waitFor(() => expect(refreshMock).toHaveBeenCalledTimes(1));
  });
    ...
    global.fetch = vi.fn().mockResolvedValue({
      ...
      json: async () => ({
        ...
        error: {
          ...
          message: `Brand library record ${RECORD_ID} cannot be published while its approval status is 'draft'`,
        },
  ...
  it("re-syncs isPublished from a fresh prop (e.g. after a sibling BrandLibraryStatusActions transition triggers router.refresh()) without a remount", () => {
