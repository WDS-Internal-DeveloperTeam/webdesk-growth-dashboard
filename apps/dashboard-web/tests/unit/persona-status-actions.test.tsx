...
import { PersonaStatusActions } from "../../components/persona-status-actions.js";

const PERSONA_ID = "11111111-1111-1111-1111-111111111111";

describe("PersonaStatusActions", () => {
  ...
  it("draft: renders Submit for Review and Archive", () => {
    render(<PersonaStatusActions personaId={PERSONA_ID} approvalStatus="draft" />);
    expect(screen.getByRole("button", { name: "Submit for Review" })).toBeInTheDocument();
    expect(screen.getByRole("button", { name: "Archive" })).toBeInTheDocument();
  });

  it("under_review: renders Approve, Request Revision, Reject, and Archive", () => {
    render(<PersonaStatusActions personaId={PERSONA_ID} approvalStatus="under_review" />);
  ...
  it("rejected: includes Revert to Draft — the fully-recoverable resubmit path, no confirmation needed", () => {
    render(<PersonaStatusActions personaId={PERSONA_ID} approvalStatus="rejected" />);
  ...
  it("archived: renders nothing — archived is a terminal state", () => {
    const { container } = render(
      <PersonaStatusActions personaId={PERSONA_ID} approvalStatus="archived" />,
    );
  ...
  it("superseded: renders nothing — superseded is a terminal state", () => {
    const { container } = render(
      <PersonaStatusActions personaId={PERSONA_ID} approvalStatus="superseded" />,
    );
    expect(container).toBeEmptyDOMElement();
  });
  ...
  it("Submit for Review: posts {approvalStatus: submitted} with no confirmation prompt, then refreshes", async () => {
    ...
    expect(fetchMock).toHaveBeenCalledWith(
      `https://api.example.com/persona-library/personas/${PERSONA_ID}/status`,
    ...
    global.fetch = vi.fn().mockResolvedValue({
      ...
      json: async () => ({
        ...
        error: {
          ...
          message: `Persona ${PERSONA_ID} approval status changed concurrently (expected draft, now submitted) — reload and retry`,
        },
  ...
  it("renders nothing for a status outside the known union, instead of throwing", () => {
    const unknownStatus = "not_a_real_status" as unknown as Parameters<
      typeof PersonaStatusActions
    >[0]["approvalStatus"];
    const { container } = render(
      <PersonaStatusActions personaId={PERSONA_ID} approvalStatus={unknownStatus} />,
    );
    expect(container).toBeEmptyDOMElement();
  });
});
