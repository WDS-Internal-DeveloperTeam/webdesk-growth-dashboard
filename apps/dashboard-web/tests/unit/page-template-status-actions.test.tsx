...
import { PageTemplateStatusActions } from "../../components/page-template-status-actions.js";

const RECORD_ID = "11111111-1111-1111-1111-111111111111";

describe("PageTemplateStatusActions", () => {
  ...
  it("draft: renders Submit for Review and Archive", () => {
    render(<PageTemplateStatusActions recordId={RECORD_ID} approvalStatus="draft" />);
    expect(screen.getByRole("button", { name: "Submit for Review" })).toBeInTheDocument();
    expect(screen.getByRole("button", { name: "Archive" })).toBeInTheDocument();
  });

  it("under_review: renders Approve, Request Revision, Reject, and Archive", () => {
    render(<PageTemplateStatusActions recordId={RECORD_ID} approvalStatus="under_review" />);
  ...
  it("approved: renders ONLY Archive — this module's own deliberate divergence from every sibling module, which also offers Mark as Superseded here", () => {
    render(<PageTemplateStatusActions recordId={RECORD_ID} approvalStatus="approved" />);
    expect(screen.getByRole("button", { name: "Archive" })).toBeInTheDocument();
  ...
  it("rejected: includes Revert to Draft — the fully-recoverable resubmit path, no confirmation needed", () => {
    render(<PageTemplateStatusActions recordId={RECORD_ID} approvalStatus="rejected" />);
  ...
  it("archived: renders nothing — archived is a terminal state", () => {
    const { container } = render(
      <PageTemplateStatusActions recordId={RECORD_ID} approvalStatus="archived" />,
    );
  ...
  it("superseded: renders nothing — superseded is a terminal state", () => {
    const { container } = render(
      <PageTemplateStatusActions recordId={RECORD_ID} approvalStatus="superseded" />,
    );
    expect(container).toBeEmptyDOMElement();
  });
  ...
  it("Submit for Review: posts {approvalStatus: submitted} with no confirmation prompt, then refreshes", async () => {
    ...
    expect(fetchMock).toHaveBeenCalledWith(
      `https://api.example.com/page-template-library/page-templates/${RECORD_ID}/status`,
    ...
    global.fetch = vi.fn().mockResolvedValue({
      ...
      json: async () => ({
        ...
        error: {
          ...
          message: `Page template ${RECORD_ID} approval status changed concurrently (expected draft, now submitted) — reload and retry`,
        },
