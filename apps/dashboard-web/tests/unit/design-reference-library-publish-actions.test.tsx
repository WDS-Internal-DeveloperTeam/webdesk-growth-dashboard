...
import { DesignReferenceLibraryPublishActions } from "../../components/design-reference-library-publish-actions.js";

const RECORD_ID = "11111111-1111-1111-1111-111111111111";

describe("DesignReferenceLibraryPublishActions", () => {
  ...
  it("Publish: posts to the publish route with no confirmation prompt, then refreshes", async () => {
    ...
    expect(fetchMock).toHaveBeenCalledWith(
      `https://api.example.com/design-reference-library/records/${RECORD_ID}/publish`,
  ...
  it("Unpublish: posts to the unpublish route with no confirmation prompt (still approved), then refreshes", async () => {
    ...
    expect(confirmSpy).not.toHaveBeenCalled();
    expect(fetchMock).toHaveBeenCalledWith(
      `https://api.example.com/design-reference-library/records/${RECORD_ID}/unpublish`,
      expect.objectContaining({ method: "POST", credentials: "include" }),
    );
    await waitFor(() => expect(refreshMock).toHaveBeenCalledTimes(1));
  });
    ...
    global.fetch = vi.fn().mockResolvedValue({
      ...
      json: async () => ({
        ...
        error: {
          ...
          message: `Design reference record ${RECORD_ID} cannot be published while its approval status is 'draft'`,
        },
  ...
  it("re-syncs isPublished from a fresh prop (e.g. after a sibling DesignReferenceLibraryStatusActions transition triggers router.refresh()) without a remount", () => {
