...
import { APPROVAL_STATUS_LABEL, APPROVAL_STATUS_VALUES } from "@/lib/portfolio-library-query";
import {
...
interface PortfolioLibraryListPageProps {
 ...
 * exactly what `GET /portfolio-library/records` returns and supports (an `approvalStatus` filter,
 * an `isPublished` filter, `search`, and offset pagination, no sort, no `recordType` filter —
 * unlike Brand Library, this module has no discriminator), matching every sibling list page's own
 * "smallest honest reading" precedent for an unsourced screen.
 */
export default async function PortfolioLibraryListPage({
  searchParams,
}: PortfolioLibraryListPageProps) {
  ...
  const query = parsePortfolioLibrarySearchParams(await searchParams);
  const { items: records, hasNextPage } = await getPortfolioRecords(query);
  ...
  return (
    <ContentContainer>
      <PageHeader
        ...
        contextActions={
          <Link href="/portfolio-library/new" style={primaryActionLinkStyle}>
            New record
          </Link>
        }
      />
      ...
      >
        ...
        <label style={{ display: "flex", flexDirection: "column", gap: "0.25rem" }}>
          ...
          {/* `key` forces a remount whenever the underlying filter value changes, including back
              ...
              on first mount (see design-reference-library/page.tsx's own identical note). */}
          <select
      ...
      {records.length === 0 ? (
        <EmptyState
          title={
            isPastLastPage
              ? "No more portfolio records"
              : hasFilters || query.search
                ? "No portfolio records match your filters"
          ...
          description={
            isPastLastPage
              ...
              : hasFilters || query.search
                ...
                : "Portfolio records created for this organization will appear here."
          }
          action={
            isPastLastPage ? (
              <Link
                href={buildPortfolioLibraryHref(query, {
              ...
              <Link href="/portfolio-library" style={{ fontSize: "0.875rem" }}>
                Clear filters
              </Link>
            ) : undefined
          }
        ...
        <>
          <div style={{ overflowX: "auto" }}>
            <table style={{ width: "100%", borderCollapse: "collapse", fontSize: "0.875rem" }}>
              <thead>
                <tr>
                  <th style={thStyle}>Project/client</th>
                  <th style={thStyle}>Primary category</th>
              ...
              <tbody>
                {records.map((record) => (
                  <PortfolioRow key={record.id} record={record} />
                ))}
              </tbody>
            </table>
          </div>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: "1.25rem" }}>
              <PageSizeSelect
                ...
                hrefBySize={buildHrefBySize((pageSize) =>
                  buildPortfolioLibraryHref(query, { pageSize }),
                )}
              />
...
function PortfolioRow({ record }: { readonly record: PortfolioRecord }) {
  ...
  return (
    <tr>
      <td style={tdStyle}>
        <Link href={`/portfolio-library/${record.id}`}>{record.projectOrClientName}</Link>
      </td>
      <td style={tdStyle}>{record.primaryCategory ?? "—"}</td>
      <td style={tdStyle}>
