...
interface DesignTokenLibraryListPageProps {
  readonly searchParams: Promise<Record<string, string | string[] | undefined>>;
}

/**
 * No approved wireframe/screen spec exists for this module — renders exactly what
 * `GET /design-token-library/tokens` returns and supports (CURRENT versions only, a `group`
 ...
 */
export default async function DesignTokenLibraryListPage({
  searchParams,
}: DesignTokenLibraryListPageProps) {
  ...
  const query = parseDesignTokenLibrarySearchParams(await searchParams);
  const { items: tokens, hasNextPage } = await getDesignTokens(query);
  const hasFilters = query.group !== null || query.approvalStatus !== null;
  const isPastLastPage = tokens.length === 0 && query.offset > 0;
  ...
  return (
    <ContentContainer>
      <PageHeader
        ...
        contextActions={
          <Link href="/design-token-library/new" style={primaryActionLinkStyle}>
            New token
          </Link>
        }
      />
        ...
        <EmptyState
          ...
          description={
            isPastLastPage
              ...
              : hasFilters || query.search
                ? "Try a different group, status, or search term."
                : "Tokens created for this organization will appear here."
          }
          action={
            isPastLastPage ? (
              <Link
                href={buildDesignTokenLibraryHref(query, {
              ...
              <Link href="/design-token-library" style={{ fontSize: "0.875rem" }}>
                Clear filters
              </Link>
            ) : undefined
          }
        ...
        <>
          ...
          >
            <span style={{ color: "var(--webdesk-dashboard-color-foreground-muted)" }}>
              Showing {query.offset + 1}–{query.offset + tokens.length}
            </span>
            <div style={{ display: "flex", alignItems: "center", gap: "1.25rem" }}>
              <PageSizeSelect
                value={query.pageSize}
                hrefBySize={buildHrefBySize((pageSize) =>
                  buildDesignTokenLibraryHref(query, { pageSize }),
                )}
              />
...
function TokenRow({ token }: { readonly token: DesignTokenRecord }) {
  const approvalBadge = designTokenApprovalStatusBadge(token.approvalStatus);
  return (
    <tr>
      <td style={tdStyle}>
        <Link href={`/design-token-library/${token.recordId}`}>{token.name}</Link>
      </td>
      <td style={{ ...tdStyle, color: "var(--webdesk-dashboard-color-foreground-muted)" }}>
        {GROUP_LABEL[token.group]}
      </td>
      <td style={{ ...tdStyle, color: "var(--webdesk-dashboard-color-foreground-muted)" }}>
        {token.value}
      </td>
