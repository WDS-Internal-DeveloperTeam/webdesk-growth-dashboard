...
/**
 * `AuditLogsAndSystemHealthQuery`/`parse.../build...`/labels live in their own file with zero
 * non-type imports, matching `lib/decision-and-activity-log-query.ts`'s own precedent, so a future
 * client component could import these directly without pulling in
 * `lib/audit-logs-and-system-health.ts`'s `next/headers` import.
 *
 * Mirrors `apps/dashboard-api/src/audit-logs-and-system-health/audit-logs-and-system-health.constants.ts`'s
 * `AUDIT_LOGS_AND_SYSTEM_HEALTH_EVENT_TYPES` — kept in sync by hand, same approach every sibling
...
export const AUDIT_LOGS_AND_SYSTEM_HEALTH_EVENT_TYPE_LABEL: Readonly<
...
export function auditLogsAndSystemHealthEventTypeLabel(eventType: AuditEventType): string {
  return AUDIT_LOGS_AND_SYSTEM_HEALTH_EVENT_TYPE_LABEL[eventType] ?? eventType;
}

function isKnownEventType(value: string): value is AuditEventType {
  return (AUDIT_LOGS_AND_SYSTEM_HEALTH_EVENT_TYPES as readonly string[]).includes(value);
}

export interface AuditLogsAndSystemHealthQuery {
...
/**
 * The backend's own query schema (`listAuditLogsAndSystemHealthEventsQuerySchema`) supports a
 * repeated `eventType` (an array), `projectId`/`actorUserId` as real UUIDs, and `from`/`to` as
 * full ISO-8601 datetimes. This UI deliberately narrows to a single `eventType` selection and a
 * plain `<input type="date">` for `from`/`to`, converted to a full UTC-boundary ISO datetime at
 * request time (`toStartOfDayIso`/`toEndOfDayIso` in `lib/audit-logs-and-system-health.ts`) rather
 * than at parse time here, so the raw date string round-trips cleanly through the URL/form
 * `defaultValue` without a timezone-conversion lossy step in between — mirrors Decision and
 * Activity Log's own list page precedent exactly.
 *
 ...
 */
export function parseAuditLogsAndSystemHealthSearchParams(
  raw: Record<string, string | string[] | undefined>,
): AuditLogsAndSystemHealthQuery {
...
/**
 * Builds a `/audit-logs-and-system-health?...` href — `overrides` wins over `current`, and
 * changing anything other than `offset` itself resets `offset` to 0, matching every sibling list
 * page's own `buildXHref` convention.
 */
export function buildAuditLogsAndSystemHealthHref(
  current: AuditLogsAndSystemHealthQuery,
  overrides: Partial<AuditLogsAndSystemHealthQuery>,
): string {
  const next: AuditLogsAndSystemHealthQuery = {
    ...current,
    ...overrides,
  ...
  return queryString
    ? `/audit-logs-and-system-health?${queryString}`
