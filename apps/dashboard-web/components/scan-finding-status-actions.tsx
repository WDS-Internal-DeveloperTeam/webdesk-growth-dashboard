...
import type { ScanFinding, ScanFindingStatus } from "@webdesk/shared-types";
import { postMutation } from "@/lib/api-errors";
import { getApiBaseUrl } from "@/lib/auth";
import { useSyncedState } from "@/lib/use-synced-state";
import styles from "./scan-finding-status-actions.module.css";

export interface ScanFindingStatusActionsProps {
 ...
 * `apps/dashboard-api/src/scan-center/scan-findings.service.ts` — kept in sync by hand, the same
 * convention every sibling status-actions component in this app already uses. Only the legal
 * target statuses are mirrored here (the required RBAC action — `review` for every real
 * transition here — is checked dynamically server-side, matching `ScanRunStatusActions`'s own
 * identical layered-guard reasoning).
 *
 ...
 */
const ALLOWED_TRANSITIONS: Readonly<Record<ScanFindingStatus, readonly ScanFindingStatus[]>> = {
...
const ACTION_LABEL: Readonly<Record<ScanFindingStatus, string>> = {
  open: "Reopen",
 ...
 *  4-state workflow — so both get a confirmation, matching `ProjectStatusActions`'s own "confirm
 *  only the transition the state machine can never reverse" precedent. */
const CONFIRM_STATUSES: ReadonlySet<ScanFindingStatus> = new Set(["resolved", "dismissed"]);

/**
 * Status-transition actions for the scan finding detail page's header — a bespoke 4-state
 ...
 * sibling status-actions component from using it applies here: `changeScanFindingStatusSchema`
 ...
 */
export function ScanFindingStatusActions({
  projectId,
  findingId,
  status: initialStatus,
}: ScanFindingStatusActionsProps): ReactNode {
  const router = useRouter();
  const [status, setStatus] = useSyncedState(initialStatus);
  const [error, setError] = useState<string | null>(null);
  const [pending, setPending] = useState<ScanFindingStatus | null>(null);
  ...
  async function handleTransition(nextStatus: ScanFindingStatus): Promise<void> {
    ...
    try {
      const result = await postMutation<ScanFinding>(
        `${getApiBaseUrl()}/scan-center/projects/${projectId}/findings/${findingId}/status`,
        { status: nextStatus },
      );
      if (!result.ok) {
      ...
      console.error("Failed to change scan finding status", err);
