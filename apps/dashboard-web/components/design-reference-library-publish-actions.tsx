...
/**
 * Mirrors `BrandLibraryPublishActions`/`ContentTemplatePublishActions` exactly — `isPublished`/
 * `publishedAt` are orthogonal to `approvalStatus`, gated only inside
 * `DesignReferenceLibraryService.publish()` itself (`approvalStatus === "approved"` required;
 * `unpublish()` has no such gate — always offered once published, even against a record that later
 * moved to `archived`/`superseded`, per D8, reusing Brand Library's own identical asymmetry). This
 ...
 * point, exactly like every sibling `*StatusActions`/`*PublishActions` component's own relationship
 * to its backend logic.
 *
 ...
 * `BrandLibraryPublishActions`'s/`ContentTemplatePublishActions`'s own identical reasoning.
 *
 ...
 */
export function DesignReferenceLibraryPublishActions({
  recordId,
  approvalStatus,
  ...
  // DesignReferenceLibraryStatusActions reserves for its own terminal-state transitions.
  const IRREVERSIBLE_UNPUBLISH_MESSAGE =
    "Unpublish this design reference record? Its approval status is " +
  ...
  async function handle(action: "publish" | "unpublish"): Promise<void> {
    ...
    try {
      const result = await postMutation(
        `${getApiBaseUrl()}/design-reference-library/records/${recordId}/${action}`,
      );
      if (!result.ok) {
        setError(result.message);
        return;
      }
      // Same batched-render pattern DesignReferenceLibraryStatusActions/every sibling
      // *PublishActions component uses: update the rendered button set from the just-confirmed
      // transition immediately, rather than waiting on router.refresh() to reconcile it.
      setIsPublished(action === "publish");
      router.refresh();
    } catch (err) {
      console.error(`Failed to ${action} design reference record`, err);
