 ...
 * `apps/dashboard-api/src/design-reference-library/design-reference-library.service.ts` — kept in
 * sync by hand, the same approach `BrandLibraryStatusActions`/`ContentTemplateStatusActions`/
 * `PersonaStatusActions`/`ServiceStatusActions`/`ProjectStatusActions`/
 * `BusinessKnowledgeStatusActions` already use. Reused verbatim from Brand Library's own identical
 * 8-state workflow, since the backend's own `TRANSITIONS` table is itself a direct copy — only the
 * legal target statuses are mirrored here (not the required RBAC action per transition); the
 * backend's own `AuthorizationService.assertAllowed()` call remains the authoritative check either
 * way, and a caller without the real grant still gets a clean 403, shown via the same
 * `parseApiErrorMessage()` allowlist every mutation in this app uses. `superseded`/`archived` are
 * both terminal (empty target lists).
 *
 * This is now the 7th independent hand-copy of this exact pattern (after `ProjectStatusActions`,
 * `BusinessKnowledgeStatusActions`, `ServiceStatusActions`, `PersonaStatusActions`,
 * `ContentTemplateStatusActions`, `BrandLibraryStatusActions`) — accepted, tracked debt, same
...
const ALLOWED_TRANSITIONS: Readonly<
  Record<DesignReferenceApprovalStatus, readonly DesignReferenceApprovalStatus[]>
> = {
 ...
 *  sibling `*StatusActions.ACTION_LABEL` establishes. */
const ACTION_LABEL: Readonly<Record<DesignReferenceApprovalStatus, string>> = {
...
const CONFIRM_MESSAGE: Partial<Record<DesignReferenceApprovalStatus, string>> = {
  archived: "Archive this design reference record? Archived records can't be reactivated.",
  superseded:
    "Mark this design reference record as superseded? Superseded records can't be reactivated.",
};

/**
 * Status-transition actions for the design reference library detail page's header — the real
 * 8-value `approvalStatus` workflow, reused verbatim from Brand Library's own. No approved design
 * brief names an `ApprovalBlock` component for this module, and the same reasoning that kept every
 * sibling `*StatusActions` component from using it applies identically here:
 * `changeDesignReferenceApprovalStatusSchema` accepts only `{approvalStatus}`, no submitter/
 * reviewer identity or rejection/revision reason, so a component expecting that data would need to
 * fabricate it. Renders only the transitions `POST .../:id/status` would actually accept from the
 ...
 */
export function DesignReferenceLibraryStatusActions({
  recordId,
  ...
  async function handleTransition(nextStatus: DesignReferenceApprovalStatus): Promise<void> {
    ...
    try {
      const result = await postMutation(
        `${getApiBaseUrl()}/design-reference-library/records/${recordId}/status`,
      ...
      console.error("Failed to change design reference record approval status", err);
