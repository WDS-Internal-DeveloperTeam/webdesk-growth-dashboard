...
import type { SectionPatternApprovalStatus } from "@webdesk/shared-types";
import { parseApiErrorMessage } from "@/lib/api-errors";
import { getApiBaseUrl } from "@/lib/auth";
import styles from "./section-pattern-status-actions.module.css";

export interface SectionPatternStatusActionsProps {
 ...
 * `apps/dashboard-api/src/section-and-pattern-library/section-patterns.service.ts` — kept in sync
 * by hand, same approach `DesignTokenStatusActions`/`WebsiteStrategyStatusActions`/
 * `ServiceStatusActions`/`PersonaStatusActions`/`ProofClaimStatusActions` already use. This is
 * another independent hand-copy of the shared 8-value artifact-approval-status pattern —
 * accepted, tracked debt, same as the prior copies.
 *
 * DELIBERATE DIVERGENCE, identical to `DesignTokenStatusActions`'s/`WebsiteStrategyStatusActions`'s
 ...
 * (`SectionPatternsService.changeApprovalStatus()`'s own doc comment explains why a direct
...
const ALLOWED_TRANSITIONS: Readonly<
  Record<SectionPatternApprovalStatus, readonly SectionPatternApprovalStatus[]>
> = {
...
const ACTION_LABEL: Readonly<Record<SectionPatternApprovalStatus, string>> = {
...
const CONFIRM_MESSAGE: Partial<Record<SectionPatternApprovalStatus, string>> = {
  archived: "Archive this record? Archived records can't be reactivated.",
};

/**
 * Status-transition actions for the Section and Pattern Library detail page's header — the real
 * 8-value `approvalStatus` workflow, reused verbatim from Design Token Library's/Website Strategy
 * Center's/Service Library's/Persona Library's/Proof and Claims Library's own (minus the one
 ...
 * here: `changeSectionPatternApprovalStatusSchema` accepts only `{approvalStatus}`, no
 ...
 * `POST /section-and-pattern-library/records/:recordId/status` would actually accept from the
 ...
 */
export function SectionPatternStatusActions({
  recordId,
  approvalStatus: initialStatus,
}: SectionPatternStatusActionsProps): ReactNode {
  const router = useRouter();
  const [approvalStatus, setApprovalStatus] = useState(initialStatus);
  const [error, setError] = useState<string | null>(null);
  const [pending, setPending] = useState<SectionPatternApprovalStatus | null>(null);
  ...
  async function handleTransition(nextStatus: SectionPatternApprovalStatus): Promise<void> {
    ...
    try {
      const response = await fetch(
        `${getApiBaseUrl()}/section-and-pattern-library/records/${recordId}/status`,
        {
          method: "POST",
          credentials: "include",
      ...
      console.error("Failed to change section/pattern record approval status", err);
