...
import type {
  ...
  WireframeRecordListFilter,
  WireframeRecordRepository,
...
import { MODULE_KEY, WIREFRAME_RECORD_REPOSITORY } from "./wireframe-library.constants.js";
import type {
  CreateWireframeRecordDto,
  UpdateWireframeRecordDto,
 ...
 * `SectionPatternsService`'s/`WebsiteStrategyRecordsService`'s own file-local `sanitizeOrInherit()`
 * helper exactly.
 */
 ...
 *  required for a given `approvalStatus` transition — identical vocabulary to Section and Pattern
 *  Library's/Design Token Library's/Website Strategy Center's/Service Library's/Persona Library's/
 *  Proof and Claims Library's own. */
type WireframeApprovalAction = "submit" | "review" | "approve";

/**
 * Sourced from `05_Workflow_State_Machines.md §2`'s generic artifact lifecycle, reused verbatim
 * from `SectionPatternsService`'s/`DesignTokensService`'s own (already code-reviewed)
 * `TRANSITIONS` table — deliberately not extracted into a shared helper (already-accepted,
 * out-of-scope debt in this codebase). `submitted`/`revision_requested`/`rejected -> draft` all
 * require `submit` (the submitter/editor drives the revise-and-resubmit loop, not the approver).
 * `archived`/`superseded` are both terminal — no code path resurrects a record from either.
 *
 * Mirrors Section and Pattern Library's/Design Token Library's own table exactly, including the
 * one deliberate deviation from the Service/Persona/Proof-and-Claims Library copies: `approved`
 * has no `superseded` edge here either — "supersede" is never a distinct user action for a
...
const TRANSITIONS: Readonly<
  Record<
    WireframeApprovalStatus,
    Readonly<Partial<Record<WireframeApprovalStatus, WireframeApprovalAction>>>
  >
> = {
...
export class WireframesService {
  constructor(
    @Inject(WIREFRAME_RECORD_REPOSITORY)
    private readonly records: WireframeRecordRepository,
    private readonly authorizationService: AuthorizationService,
    private readonly auditService: AuditService,
    private readonly usersService: UsersService,
  ) {}

  /** Existence-validates `reviewerUserId`, throwing a clean `BadRequestException` (a caller-
   *  supplied FK failing to resolve is the caller's bad input, not a missing resource) — delegates
   *  to the shared `UsersService.assertUserExists()` helper (`module-review-and-approval-center`'s
   *  own consolidation of what `ProjectService.assertOwnerExists()`/`InternalLinksService.assertApproverExists()`
   *  had each independently hand-copied), rather than re-hand-copying that pattern a 4th time. */
  private async assertReviewerExists(reviewerUserId: string): Promise<void> {
    await this.usersService.assertUserExists(reviewerUserId, "reviewerUserId");
  }

  async create(
    input: CreateWireframeRecordDto,
    actorUserId: string,
    ...
    if (input.reviewerUserId) {
      await this.assertReviewerExists(input.reviewerUserId);
    }

    let created: WireframeRecordEntity;
    try {
      created = await this.records.create({
        ...
        annotations: sanitizeNullableRichText(input.annotations),
        interactionNotes: sanitizeNullableRichText(input.interactionNotes),
    ...
    await this.auditService.record({
      ...
      actorUserId,
      actorType: "human",
      entityType: "wireframe_record",
      entityId: created.id,
      action: "create",
      afterState: { pageOrModule: created.pageOrModule, viewport: created.viewport },
  ...
  async findCurrent(recordId: string): Promise<WireframeRecordEntity> {
    const current = await this.records.findCurrentByRecordId(recordId);
    if (!current) {
      throw new NotFoundException(`Wireframe record not found: ${recordId}`);
    }
    return current;
  }
   ...
   * version instead — `pageOrModule`/`publicId` are copied forward unchanged (immutable across a
   * record's own version chain), and any field the patch omits falls back to the current version's
   * own stored value. The response's own `versionNumber`/`isCurrent`/`id` naturally reveal which
   * path was taken (a new version is, unavoidably, a different row) — no special response wrapper
   * is needed.
   */
  async update(
    recordId: string,
    patch: UpdateWireframeRecordDto,
    actorUserId: string,
  ): Promise<WireframeRecordEntity> {
    const current = await this.findCurrent(recordId);

    if (patch.reviewerUserId) {
      await this.assertReviewerExists(patch.reviewerUserId);
    }
    ...
    if (current.approvalStatus === "archived" || current.approvalStatus === "superseded") {
      throw new BadRequestException(
        `Wireframe record ${recordId} is ${current.approvalStatus} and can no longer be edited`,
      );
    }
    ...
    if (current.approvalStatus !== "approved") {
      ...
      const updated = await this.records.updateInPlace(
        ...
        {
          ...patch,
          annotations: sanitizeNullableRichTextIfChanged(patch.annotations, current.annotations),
          interactionNotes: sanitizeNullableRichTextIfChanged(
            patch.interactionNotes,
      ...
      if (!updated) {
        ...
        throw new ConflictException(
          `Wireframe record ${recordId} approval status changed concurrently while editing — reload and retry`,
        );
      }
    ...
    // SectionPatternsService.update()'s own placement: the SERVICE layer opens withTransaction()
    ...
    try {
      created = await withTransaction(async (transaction) => {
        ...
        // concurrent approve->archived transition landing between the findCurrent() read above and
        // this write let the fork proceed anyway, resurrecting a just-archived record into a fresh
        // editable draft using only the "edit" grant, never "approve", for the resurrection half
        // of the race. archived/superseded are documented as permanently terminal — this guard is
        // what actually makes that true under concurrency, not just at the single findCurrent()
        // read.
        ...
        if (!flipped) {
          ...
          // non-approved branch's own reasoning) — in practice this means the row's approvalStatus
          // changed concurrently (e.g. it was just archived), which is exactly the race this guard
          // exists to catch. A real re-check isn't possible from inside an already-failing
          // transaction, so this is reported as a conflict, the semantically correct outcome for
        ...
        return this.records.createNewVersion(
          {
            ...
            pageOrModule: current.pageOrModule,
            versionNumber: nextVersionNumber,
            viewport: patch.viewport ?? current.viewport,
            fileReference:
              patch.fileReference !== undefined ? patch.fileReference : current.fileReference,
            annotations: sanitizeOrInherit(patch.annotations, current.annotations),
            interactionNotes: sanitizeOrInherit(patch.interactionNotes, current.interactionNotes),
            relatedTemplateId:
              patch.relatedTemplateId !== undefined
                ? patch.relatedTemplateId
                : current.relatedTemplateId,
            reviewerUserId:
              patch.reviewerUserId !== undefined ? patch.reviewerUserId : current.reviewerUserId,
            createdBy: actorUserId,
          },
          transaction,
        );
      ...
      // (record_id, version_number) unique index (migration 00084). Mirrors create()'s own
      ...
      // `isSequelizeUniqueConstraintError()` helper.
      if (isSequelizeUniqueConstraintError(error)) {
        throw new ConflictException(
          `Wireframe record ${recordId} was edited concurrently — reload and retry`,
        );
      }
      throw error;
    }
  ...
  async changeApprovalStatus(
    ...
    if (!requiredAction) {
      throw new BadRequestException(
        `Invalid wireframe approval status transition: ${current.approvalStatus} -> ${nextStatus}`,
      );
    }
    ...
    if (result.outcome === "conflict") {
      throw new ConflictException(
        `Wireframe record ${recordId} approval status changed concurrently ` +
    ...
    try {
      ...
      console.error(
        `Wireframe record ${recordId} approval status transition ` +
