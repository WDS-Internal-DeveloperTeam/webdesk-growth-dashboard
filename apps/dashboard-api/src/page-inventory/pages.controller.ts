...
@Controller("page-inventory/pages")
...
export class PagesController {
  ...
  async findOne(
    ...
    const data = await this.pages.findById(id);
    return { success: true, data, correlationId: req.correlationId ?? "unknown" };
  }
  ...
  async create(
    ...
    const data = await this.pages.create(body, req.authUser!.id);
    return { success: true, data, correlationId: req.correlationId ?? "unknown" };
  }
  ...
  async update(
    ...
    const data = await this.pages.update(id, body, req.authUser!.id);
    return { success: true, data, correlationId: req.correlationId ?? "unknown" };
  }
  ...
  async changeWorkflowStage(
    ...
    const data = await this.pages.changeWorkflowStage(id, body.workflowStage, req.authUser!.id);
    return { success: true, data, correlationId: req.correlationId ?? "unknown" };
  }
}
