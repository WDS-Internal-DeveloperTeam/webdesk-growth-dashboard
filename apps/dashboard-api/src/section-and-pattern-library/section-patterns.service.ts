...
import type {
  ...
  SectionPatternRecordListFilter,
  SectionPatternRecordRepository,
...
import {
  MODULE_KEY,
  SECTION_PATTERN_RECORD_REPOSITORY,
} from "./section-and-pattern-library.constants.js";
import type {
  CreateSectionPatternRecordDto,
  UpdateSectionPatternRecordDto,
 ...
 * `WebsiteStrategyRecordsService`'s own file-local `sanitizeOrInherit()` helper exactly.
 */
 ...
 *  Library's own. */
type SectionPatternApprovalAction = "submit" | "review" | "approve";

/**
 * Sourced from `05_Workflow_State_Machines.md §2`'s generic artifact lifecycle, reused verbatim
 * from `DesignTokensService`'s/`WebsiteStrategyRecordsService`'s/`ClaimsService`'s/`ServicesService`'s/
 * `PersonasService`'s own (already code-reviewed) `TRANSITIONS` table — deliberately not extracted
 * into a shared helper (already-accepted, out-of-scope debt in this codebase). `submitted`/
...
const TRANSITIONS: Readonly<
  Record<
    SectionPatternApprovalStatus,
    Readonly<Partial<Record<SectionPatternApprovalStatus, SectionPatternApprovalAction>>>
  >
> = {
...
@Injectable()
export class SectionPatternsService {
  constructor(
    @Inject(SECTION_PATTERN_RECORD_REPOSITORY)
    private readonly records: SectionPatternRecordRepository,
    private readonly authorizationService: AuthorizationService,
    private readonly auditService: AuditService,
  ) {}

  async create(
    input: CreateSectionPatternRecordDto,
    actorUserId: string,
    ...
    let created: SectionPatternRecordEntity;
    try {
      created = await this.records.create({
        ...
        responsiveBehavior: sanitizeNullableRichText(input.responsiveBehavior),
      ...
      // `isSequelizeUniqueConstraintError()` helper (`@webdesk/validation`), matching Design Token
      // Library's own already-fixed pattern.
    ...
    await this.auditService.record({
      ...
      actorUserId,
      actorType: "human",
      entityType: "section_pattern_record",
      entityId: created.id,
      action: "create",
      afterState: { name: created.name, patternType: created.patternType },
  ...
  async findCurrent(recordId: string): Promise<SectionPatternRecordEntity> {
    ...
    if (!current) {
      throw new NotFoundException(`Section/pattern record not found: ${recordId}`);
    }
    return current;
  }
   ...
   * version instead — `patternType`/`publicId` are copied forward unchanged (immutable across a
  ...
  async update(
    recordId: string,
    patch: UpdateSectionPatternRecordDto,
    actorUserId: string,
  ): Promise<SectionPatternRecordEntity> {
    ...
    if (current.approvalStatus === "archived" || current.approvalStatus === "superseded") {
      throw new BadRequestException(
        `Section/pattern record ${recordId} is ${current.approvalStatus} and can no longer be edited`,
      );
    }
    ...
    if (current.approvalStatus !== "approved") {
      ...
      const updated = await this.records.updateInPlace(
        current.id,
        {
          ...
          responsiveBehavior: sanitizeNullableRichTextIfChanged(
      ...
      if (!updated) {
        ...
        throw new ConflictException(
          `Section/pattern record ${recordId} approval status changed concurrently while editing — reload and retry`,
        );
      }
    ...
    // DesignTokensService.update()'s/WebsiteStrategyRecordsService.update()'s own placement: the
    ...
    try {
        ...
        return this.records.createNewVersion(
          {
            ...
            scssReference:
              patch.scssReference !== undefined ? patch.scssReference : current.scssReference,
            // undefined -> inherit current; explicit null or [] -> clear to []. Distinct from a
            // naive `patch.jsDependencies ?? current.jsDependencies`, which would wrongly treat an
            // explicit null the same as omission and silently keep the old references instead of
            // clearing them (matches DesignTokenRepository's own fixed usageReferences handling).
            jsDependencies:
              patch.jsDependencies !== undefined
                ? (patch.jsDependencies ?? [])
                : current.jsDependencies,
            responsiveBehavior: sanitizeOrInherit(
              patch.responsiveBehavior,
              current.responsiveBehavior,
            ),
            ...
            tokenReferences:
              patch.tokenReferences !== undefined
                ? (patch.tokenReferences ?? [])
                : current.tokenReferences,
            relatedComponentIds:
      ...
      // (record_id, version_number) unique index (migration 00080). Mirrors create()'s own
      ...
      if (isSequelizeUniqueConstraintError(error)) {
        throw new ConflictException(
          `Section/pattern record ${recordId} was edited concurrently — reload and retry`,
        );
      }
      throw error;
    }
  ...
  async changeApprovalStatus(
    ...
    if (!requiredAction) {
      throw new BadRequestException(
        `Invalid section/pattern approval status transition: ${current.approvalStatus} -> ${nextStatus}`,
      );
    }
    ...
    if (result.outcome === "conflict") {
      throw new ConflictException(
        `Section/pattern record ${recordId} approval status changed concurrently ` +
    ...
    // the byte-identical, already-accepted pattern DesignTokensService.changeApprovalStatus()/
    ...
    try {
      ...
      console.error(
        `Section/pattern record ${recordId} approval status transition ` +
