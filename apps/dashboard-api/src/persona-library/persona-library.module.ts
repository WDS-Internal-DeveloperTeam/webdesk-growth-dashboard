 ...
 * `PersonasService.changeApprovalStatus()`), and `AuditModule` for `AuditService` (create/update/
 * status transitions are all audited). No `UsersModule` import — unlike Service Library/Projects,
 * this module has no `ownerUserId`/`parentServiceId`-style field needing existence validation
 * (D7), and no `AuthorizationService.canViewConfidential()` usage — no confidential-field
 * mechanism exists here (D6, the module registry's own seeded `confidentialityLevel: null`).
 */
@Module({
