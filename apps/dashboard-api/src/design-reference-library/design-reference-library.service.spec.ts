 ...
 *  `DesignReferenceLibraryService.create()` rather than `instanceof`, since `dashboard-api` never
...
function record(overrides: Partial<DesignReferenceRecordEntity> = {}): DesignReferenceRecordEntity {
  return {
    id: "record-1",
    publicId: "DESIGN-REF-1",
    title: "Competitor pricing page hero",
    sourceUrl: null,
    screenshotUrl: null,
...
describe("DesignReferenceLibraryService", () => {
  let records: {
  ...
  let svc: DesignReferenceLibraryService;
  ...
  beforeEach(() => {
    ...
    svc = new DesignReferenceLibraryService(
      records as unknown as DesignReferenceRecordRepository,
  ...
  describe("create", () => {
    it("creates a design reference record after validating the publicId is free", async () => {
      records.findByPublicId.mockResolvedValue(null);
      records.create.mockResolvedValue(record());

      const result = await svc.create(
        { publicId: "DESIGN-REF-1", title: "Competitor pricing page hero" },
        "actor-1",
      );

      expect(result).toEqual(record());
      expect(auditService.record).toHaveBeenCalledWith(
        expect.objectContaining({ action: "create", entityType: "design_reference_record" }),
      );
    });
    ...
    it("rejects a duplicate publicId", async () => {
      ...
      await expect(svc.create({ publicId: "DESIGN-REF-1", title: "X" }, "actor-1")).rejects.toThrow(
        BadRequestException,
      );
    ...
    it("sanitizes rich-text fields before writing, stripping a disallowed tag", async () => {
      ...
      await svc.create(
        {
          publicId: "DESIGN-X",
          title: "X",
          likes: "<script>alert(1)</script><p>Clean use of whitespace</p>",
          dislikes: null,
        },
        "actor-1",
      );
      ...
      expect(writtenInput.likes).toBe("<p>Clean use of whitespace</p>");
      expect(writtenInput.dislikes).toBeNull();
    });
    ...
    it("translates a concurrent publicId collision into a clean 400, not a raw 500", async () => {
      ...
      await expect(svc.create({ publicId: "DESIGN-RACE", title: "X" }, "actor-1")).rejects.toThrow(
        BadRequestException,
      );
    });
    ...
    it("re-throws a non-uniqueness error from create() unchanged", async () => {
      ...
      await expect(svc.create({ publicId: "DESIGN-X", title: "X" }, "actor-1")).rejects.toBe(
        dbError,
      );
    });
  });
  ...
  describe("update", () => {
    ...
    it("sanitizes rich-text fields before writing, stripping a disallowed tag", async () => {
      ...
      await svc.update(
        "record-1",
        { motionNotes: "<script>alert(1)</script><p>Sticky header on scroll</p>" },
        "actor-1",
      );

      const [, writtenPatch] = records.update.mock.calls[0] as [string, Record<string, unknown>];
      expect(writtenPatch.motionNotes).toBe("<p>Sticky header on scroll</p>");
    });
    ...
    it("skips re-sanitizing a rich-text field the patch resends unchanged from the current stored value", async () => {
      ...
      records.findById.mockResolvedValue(
        record({ approvalStatus: "draft", likes: dirtyButUnchanged }),
      );
      records.update.mockResolvedValue(record());

      await svc.update("record-1", { likes: dirtyButUnchanged }, "actor-1");

      const [, writtenPatch] = records.update.mock.calls[0] as [string, Record<string, unknown>];
      expect(writtenPatch.likes).toBe(dirtyButUnchanged);
    });
    ...
    it("returns the repository's updated entity, with version incremented server-side", async () => {
      ...
      expect(auditService.record).toHaveBeenCalledWith(
        expect.objectContaining({ action: "update", entityType: "design_reference_record" }),
      );
    });
  });
