...
import { ComponentLibraryModule } from "../src/component-library/component-library.module.js";

/**
 * Request-level coverage for the Component Library module HTTP surface, against a REAL
 * disposable PostgreSQL database — same harness pattern as `design-token-library.e2e-spec.ts`.
 * `creative_design` has real seeded grants (`00013-seed-rbac-matrix.ts:132-140`), identical to
 * Design Token Library's own matrix (both modules reuse the group verbatim, design decision 5):
 ...
 * This module also imports `DesignTokenLibraryModule` for the real `tokenIds` existence check
 * (design decision 2) — several tests exercise that cross-module validation over real HTTP.
 */
...
describe("Component Library module endpoints (e2e, real disposable database)", () => {
  ...
  beforeAll(async () => {
    ...
    superAdminUserId = await createUserWithRole("cmp.super-admin", "super_admin");
    readOnlyUserId = await createUserWithRole("cmp.read-only", "read_only");
    marketingEditorUserId = await createUserWithRole("cmp.marketing-editor", "marketing_editor");
    ownerGrowthApproverUserId = await createUserWithRole(
      "cmp.owner-growth-approver",
      "owner_growth_approver",
    );
    designerCreativeReviewerUserId = await createUserWithRole(
      "cmp.designer-creative-reviewer",
      "designer_creative_reviewer",
    );
    qaSecurityReviewerUserId = await createUserWithRole(
      "cmp.qa-security-reviewer",
  ...
  it("rejects GET /component-library/components with 401 when there is no session cookie", async () => {
    await request(app.getHttpServer()).get("/component-library/components").expect(401);
  });

  it("allows a real super_admin session to create, read, list, and edit a component", async () => {
    const cookie = await cookieForNewSession(superAdminUserId);
    const created = await createDraftComponent(cookie, { name: "Primary Button" });
    expect(created.approvalStatus).toBe("draft");
    expect(created.versionNumber).toBe(1);
    expect(created.isCurrent).toBe(true);

    const getResponse = await request(app.getHttpServer())
      .get(`/component-library/components/${created.recordId}`)
      .set("Cookie", cookie)
      .expect(200);
    expect(getResponse.body.data.name).toBe("Primary Button");

    const listResponse = await request(app.getHttpServer())
      .get("/component-library/components")
      .set("Cookie", cookie)
      .expect(200);
    expect(
    ...
    const versionsResponse = await request(app.getHttpServer())
      .get(`/component-library/components/${created.recordId}/versions`)
      .set("Cookie", cookie)
      .expect(200);
    expect(versionsResponse.body.data).toHaveLength(1);

    const updateResponse = await request(app.getHttpServer())
      .post(`/component-library/components/${created.recordId}/update`)
      .set("Cookie", cookie)
      .set("Origin", process.env.WEB_APP_ORIGIN!)
      .send({ name: "Primary Button (revised)" })
      .expect(200);
    expect(updateResponse.body.data.name).toBe("Primary Button (revised)");
  ...
  it("denies component creation with 403 for a read_only session (only V grant, not C)", async () => {
    const cookie = await cookieForNewSession(readOnlyUserId);
    await request(app.getHttpServer())
      .post("/component-library/components")
      .set("Cookie", cookie)
      .set("Origin", process.env.WEB_APP_ORIGIN!)
      .send({ publicId: uniquePublicId("CMP"), category: "buttons", name: "Denied" })
      .expect(403);
  });

  it("allows a read_only session to list components (V grant)", async () => {
    const cookie = await cookieForNewSession(readOnlyUserId);
    await request(app.getHttpServer())
      .get("/component-library/components")
      .set("Cookie", cookie)
  ...
  it("denies component creation with 403 for owner_growth_approver (VERAPX has no C)", async () => {
  ...
  it("list() search treats a literal % as literal text, not a SQL wildcard (escapeLikePattern)", async () => {
    ...
    const wildcardMatch = await createDraftComponent(cookie, {
      name: `50% Off Alert ${uniqueSuffix}`,
    });
    const plainMatch = await createDraftComponent(cookie, {
  ...
  it("rejects component creation with 400 when publicId is already in use", async () => {
    const cookie = await cookieForNewSession(superAdminUserId);
    const publicId = uniquePublicId("CMP");
    await request(app.getHttpServer())
      .post("/component-library/components")
      .set("Cookie", cookie)
      .set("Origin", process.env.WEB_APP_ORIGIN!)
      .send({ publicId, category: "buttons", name: "First" })
      .expect(201);
    await request(app.getHttpServer())
      .post("/component-library/components")
      .set("Cookie", cookie)
      .set("Origin", process.env.WEB_APP_ORIGIN!)
      .send({ publicId, category: "buttons", name: "Second" })
      .expect(400);
  });

  it("rejects an empty update patch with 400", async () => {
    const cookie = await cookieForNewSession(superAdminUserId);
    const created = await createDraftComponent(cookie);
    await request(app.getHttpServer())
      .post(`/component-library/components/${created.recordId}/update`)
      .set("Cookie", cookie)
      .set("Origin", process.env.WEB_APP_ORIGIN!)
  ...
  it("rejects a malformed figmaReference URL scheme at the API boundary (safeHttpUrlSchema)", async () => {
    const cookie = await cookieForNewSession(superAdminUserId);
    await request(app.getHttpServer())
      .post("/component-library/components")
      .set("Cookie", cookie)
      ...
      .send({
        ...
        category: "buttons",
        name: "X",
        figmaReference: "javascript:alert(1)",
      })
      .expect(400);
  });

  // --- tokenIds real cross-module existence validation (design decision 2) ---

  it("creates a component whose tokenIds resolve to real, current design tokens", async () => {
    const cookie = await cookieForNewSession(superAdminUserId);
    const token = await createDraftToken(cookie);

    const created = await createDraftComponent(cookie, { tokenIds: [token.recordId] });
    expect(created.tokenIds).toEqual([token.recordId]);
  });

  it("rejects a component create with 400 when a tokenId does not resolve to a real design token", async () => {
    const cookie = await cookieForNewSession(superAdminUserId);
    await request(app.getHttpServer())
      .post("/component-library/components")
      .set("Cookie", cookie)
  ...
  it("rejects a component update with 400 when a patched tokenId does not resolve to a real design token", async () => {
    const cookie = await cookieForNewSession(superAdminUserId);
    const created = await createDraftComponent(cookie);
    await request(app.getHttpServer())
      .post(`/component-library/components/${created.recordId}/update`)
      .set("Cookie", cookie)
      .set("Origin", process.env.WEB_APP_ORIGIN!)
      .send({ tokenIds: [randomUUID()] })
      .expect(400);
  });

  // --- replacementRecordId real in-module existence validation ---

  it("creates a component whose replacementRecordId resolves to a real, current component", async () => {
    const cookie = await cookieForNewSession(superAdminUserId);
    const original = await createDraftComponent(cookie, { name: "Legacy Component" });
    const replacement = await createDraftComponent(cookie, {
      name: "New Component",
      replacementRecordId: original.recordId,
    });
    expect(replacement.name).toBe("New Component");
  });

  it("rejects a component create with 400 when replacementRecordId does not resolve to a real component", async () => {
    const cookie = await cookieForNewSession(superAdminUserId);
    await request(app.getHttpServer())
      .post("/component-library/components")
      .set("Cookie", cookie)
  ...
  it("designer_creative_reviewer (VCERAS) alone can drive the full submit->review->approve loop (the only role holding submit)", async () => {
    ...
    const created = await createDraftComponent(cookie, { name: "Designer Reviewer Fixture" });

    await request(app.getHttpServer())
      .post(`/component-library/components/${created.recordId}/status`)
      .set("Cookie", cookie)
      .set("Origin", process.env.WEB_APP_ORIGIN!)
  ...
  it("marketing_editor (VR only) can view and review, but is denied create, submit, and approve", async () => {
    ...
    const created = await createDraftComponent(adminCookie, { name: "Marketing Editor Fixture" });
    ...
    await request(app.getHttpServer())
      ...
      .set("Origin", process.env.WEB_APP_ORIGIN!)
      .send({ publicId: uniquePublicId("CMP"), category: "buttons", name: "Denied create" })
      .expect(403);
  ...
  it("owner_growth_approver (VERAPX, no C, no S) is denied create and denied draft->submitted, but can review and approve once submitted", async () => {
    const reviewerCookie = await cookieForNewSession(designerCreativeReviewerUserId);
    const created = await createDraftComponent(reviewerCookie, { name: "Owner Approver Fixture" });
  ...
  it("qa_security_reviewer (VR only) can view and review, but is denied create and approve", async () => {
    const adminCookie = await cookieForNewSession(superAdminUserId);
    const created = await createDraftComponent(adminCookie, { name: "QA Reviewer Fixture" });
    const reviewerCookie = await cookieForNewSession(designerCreativeReviewerUserId);
    await request(app.getHttpServer())
      .post(`/component-library/components/${created.recordId}/status`)
      .set("Cookie", reviewerCookie)
  ...
  it("rejects an invalid approval-status transition (archived -> submitted, archived is terminal) with 400", async () => {
    const cookie = await cookieForNewSession(designerCreativeReviewerUserId);
    const created = await createDraftComponent(cookie, { name: "Terminal State Fixture" });
  ...
  it("rejects a direct 'approved -> superseded' status request with 400, over real HTTP (supersede is only ever an automatic side effect of a different version's own approval)", async () => {
    const reviewerCookie = await cookieForNewSession(designerCreativeReviewerUserId);
    const created = await createDraftComponent(reviewerCookie, {
  ...
  it("rejects editing an archived component with 400, over real HTTP, without mutating it (archived/superseded are terminal)", async () => {
    const cookie = await cookieForNewSession(designerCreativeReviewerUserId);
    const created = await createDraftComponent(cookie, { name: "Terminal Edit Fixture" });
  ...
  it("returns 404 for a GET on a nonexistent component id", async () => {
    const cookie = await cookieForNewSession(superAdminUserId);
    await request(app.getHttpServer())
      .get(`/component-library/components/${randomUUID()}`)
      .set("Cookie", cookie)
      .expect(404);
  });

  it("returns 400 (not a raw 500) for a malformed component id", async () => {
    const cookie = await cookieForNewSession(superAdminUserId);
    await request(app.getHttpServer())
      .get("/component-library/components/not-a-uuid")
      .set("Cookie", cookie)
      .expect(400);
  });

  it("returns 404 for GET .../versions on a nonexistent component id", async () => {
    const cookie = await cookieForNewSession(superAdminUserId);
    await request(app.getHttpServer())
      .get(`/component-library/components/${randomUUID()}/versions`)
      .set("Cookie", cookie)
      .expect(404);
  });
  ...
  it("rejects a mutating request with no Origin header (OriginCheckGuard)", async () => {
    ...
    await request(app.getHttpServer())
      .post("/component-library/components")
      .set("Cookie", cookie)
      .send({ publicId: uniquePublicId("CMP"), category: "buttons", name: "No origin" })
      .expect(403);
  });

  // --- real versioning behavior, over real HTTP, against the real database ---

  it("editing an APPROVED component creates a genuinely new version over real HTTP, then approving the new version supersedes the old one", async () => {
    const cookie = await cookieForNewSession(designerCreativeReviewerUserId);
    const created = await createDraftComponent(cookie, {
      name: "Primary V1",
      category: "buttons",
    ...
    const currentComponent = await request(app.getHttpServer())
      .get(`/component-library/components/${created.recordId}`)
      .set("Cookie", cookie)
      .expect(200);
    expect(currentComponent.body.data.id).toBe(editResponse.body.data.id);
    expect(currentComponent.body.data.name).toBe("Primary V2 (revised)");
  });

  it("editing a non-approved component mutates it in place (no new version) over real HTTP", async () => {
    const cookie = await cookieForNewSession(superAdminUserId);
    const created = await createDraftComponent(cookie, {
