...
import { SectionAndPatternLibraryModule } from "../src/section-and-pattern-library/section-and-pattern-library.module.js";

/**
 * Request-level coverage for the Section and Pattern Library module HTTP surface, against a REAL
 * disposable PostgreSQL database — same harness pattern as `design-token-library.e2e-spec.ts`.
 * Reuses the identical `creative_design` seeded RBAC matrix (`00013-seed-rbac-matrix.ts:132-140`):
...
describe("Section and Pattern Library module endpoints (e2e, real disposable database)", () => {
  ...
  beforeAll(async () => {
    ...
    const moduleRef: TestingModule = await Test.createTestingModule({
      imports: [SectionAndPatternLibraryModule],
    })
    ...
    superAdminUserId = await createUserWithRole("spl.super-admin", "super_admin");
    readOnlyUserId = await createUserWithRole("spl.read-only", "read_only");
    marketingEditorUserId = await createUserWithRole("spl.marketing-editor", "marketing_editor");
    ownerGrowthApproverUserId = await createUserWithRole(
      "spl.owner-growth-approver",
      "owner_growth_approver",
    );
    designerCreativeReviewerUserId = await createUserWithRole(
      "spl.designer-creative-reviewer",
      "designer_creative_reviewer",
    );
    qaSecurityReviewerUserId = await createUserWithRole(
      "spl.qa-security-reviewer",
  ...
  it("rejects GET /section-and-pattern-library/records with 401 when there is no session cookie", async () => {
    await request(app.getHttpServer()).get("/section-and-pattern-library/records").expect(401);
  });
  ...
  it("allows a real super_admin session to create, read, list, and edit a record", async () => {
    ...
    const created = await createDraftRecord(cookie, { name: "Homepage hero" });
    expect(created.approvalStatus).toBe("draft");
    expect(created.versionNumber).toBe(1);
    expect(created.isCurrent).toBe(true);

    const getResponse = await request(app.getHttpServer())
      .get(`/section-and-pattern-library/records/${created.recordId}`)
      .set("Cookie", cookie)
      .expect(200);
    expect(getResponse.body.data.name).toBe("Homepage hero");

    const listResponse = await request(app.getHttpServer())
      .get("/section-and-pattern-library/records")
      .set("Cookie", cookie)
      .expect(200);
    expect(
    ...
    const versionsResponse = await request(app.getHttpServer())
      .get(`/section-and-pattern-library/records/${created.recordId}/versions`)
      .set("Cookie", cookie)
      .expect(200);
    expect(versionsResponse.body.data).toHaveLength(1);

    const updateResponse = await request(app.getHttpServer())
      .post(`/section-and-pattern-library/records/${created.recordId}/update`)
      .set("Cookie", cookie)
      .set("Origin", process.env.WEB_APP_ORIGIN!)
      .send({ name: "Homepage hero (revised)" })
      .expect(200);
    expect(updateResponse.body.data.name).toBe("Homepage hero (revised)");
  ...
  it("denies record creation with 403 for a read_only session (only V grant, not C)", async () => {
    ...
    await request(app.getHttpServer())
      ...
      .set("Cookie", cookie)
      .set("Origin", process.env.WEB_APP_ORIGIN!)
      .send({ publicId: uniquePublicId("SPL"), patternType: "trust", name: "Denied" })
      .expect(403);
  });
  ...
  it("list() search treats a literal % as literal text, not a SQL wildcard (escapeLikePattern)", async () => {
    ...
    const response = await request(app.getHttpServer())
      .get("/section-and-pattern-library/records")
      .query({ search: `50% Off Pattern ${uniqueSuffix}` })
      .set("Cookie", cookie)
      .expect(200);
  ...
  it("rejects record creation with 400 when publicId is already in use", async () => {
    const cookie = await cookieForNewSession(superAdminUserId);
    const publicId = uniquePublicId("SPL");
    await request(app.getHttpServer())
      .post("/section-and-pattern-library/records")
      .set("Cookie", cookie)
      .set("Origin", process.env.WEB_APP_ORIGIN!)
      .send({ publicId, patternType: "trust", name: "First" })
      .expect(201);
    await request(app.getHttpServer())
      .post("/section-and-pattern-library/records")
      .set("Cookie", cookie)
      .set("Origin", process.env.WEB_APP_ORIGIN!)
      .send({ publicId, patternType: "trust", name: "Second" })
      .expect(400);
  });
  ...
  it("rejects an unrecognized patternType at the API boundary (Zod enum validation)", async () => {
    const cookie = await cookieForNewSession(superAdminUserId);
    await request(app.getHttpServer())
      .post("/section-and-pattern-library/records")
      .set("Cookie", cookie)
      .set("Origin", process.env.WEB_APP_ORIGIN!)
      .send({ publicId: uniquePublicId("SPL"), patternType: "not_a_real_pattern", name: "X" })
      .expect(400);
  });
  ...
  it("designer_creative_reviewer (VCERAS) alone can drive the full submit->review->approve loop (the only role holding submit)", async () => {
    ...
    await request(app.getHttpServer())
      .post(`/section-and-pattern-library/records/${created.recordId}/status`)
      .set("Cookie", cookie)
      .set("Origin", process.env.WEB_APP_ORIGIN!)
  ...
  it("marketing_editor (VR only) can view and review, but is denied create, submit, and approve", async () => {
    ...
    await request(app.getHttpServer())
      ...
      .set("Origin", process.env.WEB_APP_ORIGIN!)
      .send({ publicId: uniquePublicId("SPL"), patternType: "trust", name: "Denied create" })
      .expect(403);
  ...
  it("returns 404 for a GET on a nonexistent record id", async () => {
    const cookie = await cookieForNewSession(superAdminUserId);
    await request(app.getHttpServer())
      .get(`/section-and-pattern-library/records/${randomUUID()}`)
      .set("Cookie", cookie)
      .expect(404);
  });
  ...
  it("returns 400 (not a raw 500) for a malformed record id", async () => {
    const cookie = await cookieForNewSession(superAdminUserId);
    await request(app.getHttpServer())
      .get("/section-and-pattern-library/records/not-a-uuid")
      .set("Cookie", cookie)
      .expect(400);
  });
  ...
  it("returns 404 for GET .../versions on a nonexistent record id", async () => {
    const cookie = await cookieForNewSession(superAdminUserId);
    await request(app.getHttpServer())
      .get(`/section-and-pattern-library/records/${randomUUID()}/versions`)
      .set("Cookie", cookie)
      .expect(404);
  });
  ...
  it("rejects a mutating request with no Origin header (OriginCheckGuard)", async () => {
    ...
    await request(app.getHttpServer())
      .post("/section-and-pattern-library/records")
      .set("Cookie", cookie)
      .send({ publicId: uniquePublicId("SPL"), patternType: "trust", name: "No origin" })
      .expect(403);
  });
  ...
  it("editing an APPROVED record creates a genuinely new version over real HTTP, then approving the new version supersedes the old one", async () => {
    ...
    const editResponse = await request(app.getHttpServer())
      ...
      .set("Cookie", cookie)
      .set("Origin", process.env.WEB_APP_ORIGIN!)
      .send({ name: "Homepage hero V2 (revised)" })
      .expect(200);
    ...
    // patternType is carried forward unchanged since the patch has no such field to send.
    expect(editResponse.body.data.patternType).toBe("homepage_storytelling");
    ...
    expect(currentRecord.body.data.name).toBe("Homepage hero V2 (revised)");
  });
  ...
  // tried here and found unreliably flaky (`DATABASE_POOL_MAX` defaulting to 2, plus how fast a
  // local Postgres round-trip is, meant the two requests usually didn't actually overlap at the
  // database layer, so the second request's INSERT often landed after the first one's UPDATE had
  // already committed, both succeeding with 200 instead of colliding) — no other `*.e2e-spec.ts`
  // file in this codebase attempts this shape of test for exactly that reason. The real
  // concurrent-edit-collision behavior (`SectionPatternsService.update()`'s `catch` block
  // translating a `SequelizeUniqueConstraintError` on `(record_id, version_number)` into a clean
  // 409) is already covered deterministically at the unit-test level via a mocked rejection — see
