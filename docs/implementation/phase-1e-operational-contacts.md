...
- **`operational_contacts`** (`00020`, no seed data — §17's own instruction) — full §17 field
...
- **`incident_severity_policies`** (`00021`, seeded by `00022`) — 4 rows, one per §18's approved
...
This resolves _who_ to contact from stored configuration only — it sends nothing and tracks no
...
`GET /operational-contacts/escalation-chain` (declared _before_ `:id` — NestJS matches routes in
...
56/56 `packages/database` tests (unit + integration, including a full migration `00001`→`00022`
