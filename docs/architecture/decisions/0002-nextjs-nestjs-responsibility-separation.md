...
- `dashboard-api` (NestJS) owns, relative to `dashboard-web`: all *synchronous, request-triggered* business logic, all database access via `packages/database`, all authorization checks, all integration calls (GitHub, WordPress, Google Workspace), and all webhook receivers. It does **not** own the dashboard's *asynchronous* background-job logic — that is `dashboard-worker`'s domain (ADR-0004), using the same shared `packages/database` and `packages/integrations`, not a `dashboard-api`-owned monopoly.
