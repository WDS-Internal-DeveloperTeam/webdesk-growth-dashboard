...
This ADR is scoped to the **web-vs-API framework boundary** specifically (Next.js vs. NestJS) — `dashboard-worker`'s asynchronous/scheduled role is a separate boundary, covered by ADR-0004, and is not restricted by anything below.
...
- `dashboard-api` (NestJS) owns, relative to `dashboard-web`: all _synchronous, request-triggered_ business logic, all database access via `packages/database`, all authorization checks, all integration calls (GitHub, WordPress, Google Workspace), and all webhook receivers. It does **not** own the dashboard's _asynchronous_ background-job logic — that is `dashboard-worker`'s domain (ADR-0004), using the same shared `packages/database` and `packages/integrations`, not a `dashboard-api`-owned monopoly.
