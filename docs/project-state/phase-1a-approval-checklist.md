...
| Apply proposed upstream patches         | **Not done.** No patch content was applied. `proposed-upstream-patches/` files (4 of them) were touched only by the repo-wide Prettier reformat (`git diff --ignore-all-space` shows 0 lines changed) — same cosmetic pass applied to 48 other pre-existing docs, not a content edit. |
