...
/**
 * Release Center (module `release_center`, `docs/implementation/module-release-center.md`) — a
 * real, named 14-status workflow (`05_Workflow_State_Machines.md §10`) across six project-scoped
 * tables: `releases` (the parent), `release_artifacts` ("repositories and SHAs, PRs"),
 * `release_approvals` (an append-only decision log, auto-inserted whenever a transition's own
 * action is `approve`), `deployments` (an append-only history of every deploy attempt),
 * `smoke_tests` (append-only smoke-test results), and `rollback_records` (at most one per release,
 * auto-inserted on any `-> rolled_back` transition). Mirrors
 * `packages/database/src/release-center/entities.ts` exactly. `notes`/`hotfixReason`/every
 * sub-resource's own free-text field are deliberately plain, unsanitized text — the backend never
 * sanitizes these as HTML, so this module was never in scope for the 2026-08-22 rich-text standing
 * rule.
 */
 ...
 * `COALESCE(column, NOW())` write — never accepted as caller input.
 */
...
/** "Repositories and SHAs, PRs" — create/list/delete only (delete rejected once the parent release
 *  is `completed`/`rolled_back`). `prUrl` is validated as a safe http(s) URL server-side. */
 ...
 *  own CAS status write whenever a transition's action is `approve`. Read-only via
 *  `GET .../releases/:id/approvals`. `notes` is plain, unsanitized text. */
 ...
 *  `Release.stagingDeployedAt`/`productionDeployedAt` are first stamped, since those columns
 ...
 * release's own CAS status write on any `-> rolled_back` transition. At most one per release, read
 * only via `GET .../releases/:id/rollback` (a clean 404 when the release has never been rolled
 * back). `reason` is plain, unsanitized text.
 */
